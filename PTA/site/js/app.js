...
async function handleLookup() {
  ...
  if (isPast) {
    ...
    `;
    reportPrediction = {
      data_type: 'observed 2026 record',
      probability: null,
      risk_band: level + ' observed activity',
      predicted_scale: null,
      model_auc: null,
      limitations: ['This is a recorded observation for the modeled grid cell, not a probability forecast.'],
    };
  } else {
    ...
    `;
    reportPrediction = {
      data_type: 'random-forest next-day prediction',
      probability: proba,
      risk_band: band,
      predicted_scale: levelPred,
      model_auc: state.featureMeta.holdout_auc,
      limitations: [
        `The statistical model uses a ~1-degree grid and the nearest cell is about ${Math.round(distMiles)} miles from the requested point.`,
        `Its bundled weather input is seasonal rather than a live forecast after ${cutoff}.`,
      ],
    };
  }
  ...
  wireLLMBox({
    address: geo.matchedAddress, dateStr, isPast, cell, causes,
    latitude: geo.lat, longitude: geo.lon, prediction: reportPrediction,
  });
}
...
// ---------------------------------------------------------------------------
// Optional local Qwen3-14B grounded report
// ---------------------------------------------------------------------------
function renderLLMBox() {
  return `
    <div class="llm-box">
      <h4 style="margin-top:0">Ask Qwen3-14B for a grounded fire report</h4>
      <p class="small-note">The local Qwen service retrieves public weather and active NWS alerts, then writes from those sources and the statistical prediction above. Qwen explains the result; the validated random forest remains the probability engine.</p>
      <textarea id="report-question" rows="3" placeholder="What would you like the report to address?">How likely is a fire near this location, what evidence affects the estimate, and what should residents do?</textarea>
      <button id="generate-report-btn">Fetch current data &amp; generate report</button>
...
function wireLLMBox(ctx) {
  ...
  btn.addEventListener('click', async () => {
    ...
    out.textContent = 'Fetching public data and loading Qwen3-14B… The first report can take several minutes.';
    btn.disabled = true;
    try {
      const res = await fetch('/api/report', {
        ...
        body: JSON.stringify({
          question: question || 'Summarize the wildfire risk for this place and date.',
          address: ctx.address,
          latitude: ctx.latitude,
          longitude: ctx.longitude,
          date: ctx.dateStr,
          nearby_causes: ctx.causes.causeNames,
      ...
      out.textContent = 'Could not generate the Qwen report (' + e.message + '). Start the local Qwen server and try again.\n\n' + templateReport(ctx);
    } finally {
      btn.disabled = false;
    }
  });
